...
import { generateRetrospective } from "@/lib/agents";
...
export async function POST() {
  ...
  try {
    ...
    const text = await generateRetrospective(session.raceConfig, activities);
    const retrospective = { text, generatedAt: new Date().toISOString() };
    setSession({ ...session, retrospective });
    return NextResponse.json({ retrospective });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message ?? "Failed to generate retrospective" }, { status: 500 });
  }
}

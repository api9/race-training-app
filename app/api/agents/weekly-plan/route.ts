...
import { generateWeeklyPlan } from "@/lib/agents";
...
export async function POST() {
  ...
  try {
    ...
    const text = await generateWeeklyPlan(session.raceConfig, activities);
    const weeklyPlan = { text, generatedAt: new Date().toISOString() };
    setSession({ ...session, weeklyPlan });

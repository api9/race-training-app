...
import { generateWeekOverWeekComparison } from "@/lib/agents";
...
export async function POST() {
  ...
  try {
    ...
    const result = await generateWeekOverWeekComparison(session.raceConfig, activities);
    const weekOverWeek = { ...result, generatedAt: new Date().toISOString() };
    setSession({ ...session, weekOverWeek });
    return NextResponse.json({ weekOverWeek });
  } catch (err: any) {

...
import { generateRecoverySignal } from "@/lib/agents";
...
export async function POST() {
  ...
  try {
    ...
    const result = await generateRecoverySignal(session.raceConfig, activities);
    const recoverySignal = { ...result, generatedAt: new Date().toISOString() };
    setSession({ ...session, recoverySignal });
    return NextResponse.json({ recoverySignal });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message ?? "Failed to generate recovery signal" }, { status: 500 });
  }
}
